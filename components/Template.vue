<template>
  <v-container>
    <v-row class="text-center">
      <v-col cols="12">
        <Logo/>
      </v-col>
      <v-col class="mb-4">
        <h1 class="display-2 font-weight-bold mb-3 text__primary">
          Template para Landings
        </h1>

        <p class="subheading font-weight-regular text__primary">
          Template elaborado con Nuxt + vuejs
          <br />para el desarrollo ágil de proyectos en Acceso Crediticio
        </p>
        <v-btn
          @click="openModalText"
          color="#34495E"
          class="content__primary text__secondary mb-2"
          >Modal Text</v-btn
        >
        <v-btn
          @click="openModalAlert"
          color="#34495E"
          class="content__primary text__secondary mb-2"
          >Modal Alert</v-btn
        >
      </v-col>

      <v-col cols="12">
        <h2 class="headline font-weight-bold mb-3 text__primary">
          Entorno de Trabajo
        </h2>
        <p v-if="entorno" class="text__primary mb-0">{{ entorno }}</p>
      </v-col>

      <v-col class="mb-5" cols="12">
        <h2 class="headline font-weight-bold mb-3 text__primary">
          Ecosistema
        </h2>

        <v-row justify="center">
          <a
            v-for="(eco, i) in ecosystem"
            :key="i"
            :href="eco.href"
            class="subheading mx-3 text__primary"
            target="_blank"
          >
            {{ eco.text }}
          </a>
        </v-row>
      </v-col>
    </v-row>
  </v-container>
</template>

<script>
import Logo from '~/components/Utils/Logo'
export default {
  name: "Template",
  props: ["entorno"],
  components: {
    Logo
  },
  data: () => ({
    ecosystem: [
      {
        text: "vue",
        href: "https://vuejs.org/"
      },
      {
        text: "vuex",
        href: "https://vuex.vuejs.org/"
      },
      {
        text: "vuetify",
        href: "https://vuetifyjs.com/en/"
      },
      {
        text: "axios",
        href: "https://github.com/axios/axios"
      },
      {
        text: "gitlab",
        href: "http://gitlab.acr.pe/devweb360/ld-template-js.git"
      }
    ]
  }),
  mounted() {
    this.getNews();
  },
  methods: {
    openModalText() {
      this.$store.dispatch("ui/setModalText", {
        isModal: true,
        titulo: "Título del Modal",
        textoDefaultCliente: "Contenido del Modal"
      });
    },
    openModalAlert() {
      this.$store.dispatch("ui/setModalAlert", {
        imgModal: "error",
        tituloModal: "¡Lo sentimos!",
        msgModal: "Ejemplo de modal de error",
        maxWidth: 350,
        isModal: true,
        push: "/"
      });
    },
    getNews(){
       this.$store.dispatch("template/defaultFunction").finally(() => {
         console.log("Culminó la acción");
       });
    }
  }
};
</script>
<style scoped>
.color--text {
  color: white !important;
}
</style>
